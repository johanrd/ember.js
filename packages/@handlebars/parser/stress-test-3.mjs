...
for (let h in Helpers) {
  if (Object.prototype.hasOwnProperty.call(Helpers, h)) b[h] = Helpers[h];
}
function jison(input) {
  jisonParser.yy = b;
  jisonParser.yy.locInfo = (l) => new Helpers.SourceLocation(undefined, l);
  jisonParser.yy.syntax = {
    square: 'string',
    hash: (h, l) => ({ type: 'HashLiteral', pairs: h.pairs, loc: l }),
  };
...
function compare(tpl, label) {
  ...
  try {
    v = v2ParseWithoutProcessing(tpl);
  } catch (e) {
    vErr = e;
  }
  if (jErr && vErr) {
  ...
  if (!!jErr !== !!vErr) {
    ...
    if (failures.length < 30)
      failures.push({
        label,
        issue: 'error mismatch',
        jison: jErr ? 'ERR' : 'OK',
        v2: vErr ? 'ERR: ' + vErr.message?.substring(0, 60) : 'OK',
      });
    return;
  }
...
function maybe(fn, prob = 0.5) {
  return Math.random() < prob ? fn() : '';
}
...
function randomMustache() {
  ...
  if (kind < 0.6) {
    ...
    const params = maybe(
      () =>
        ' ' + Array.from({ length: Math.floor(Math.random() * 3) + 1 }, () => pick(vals)).join(' '),
      0.4
    );
...
console.log(
  `Fuzzed ${FUZZ_COUNT} random templates: ${total - fuzzStart - fuzzErrors} compared, ${fuzzErrors} crashed\n`
);
...
compare(
  Array.from({ length: 100 }, (_, i) => `{{! comment ${i} }}`).join(''),
  'patho: 100 adjacent comments'
);

// Many adjacent blocks
compare(
  Array.from({ length: 50 }, (_, i) => `{{#x${i}}}y{{/x${i}}}`).join(''),
  'patho: 50 adjacent blocks'
);
...
compare(
  '{{helper (sub1 (sub2 arg1 key1=(sub3 arg2)) arg3) key2=(sub4 (sub5 arg4 key3=val3) arg5) key4="../path"}}',
  'patho: deeply nested mixed features'
);

// Sub-expression as path then more sub-expressions
compare(
  '{{(helper arg).prop.deep (other).thing key=(yet-another val).result}}',
  'patho: sub-expr paths everywhere'
);

// String with many escaped quotes
compare(
  '{{foo "say \\"hello\\" and \\"goodbye\\" and \\"maybe\\""}}',
  'patho: many escaped quotes in string'
);
...
compare(
  ...
  {{/if}}
`.trim(),
  'patho: deeply nested multi-line with everything'
);
